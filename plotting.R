...
temp_plot <- bind_rows(hanoi_plot, stpeter_plot, sf_plot, oakland_plot, swarthmore_plot, victoria_plot)

ggplot(temp_plot, aes(x=WEEK, y=TMAX_mean, group=CITY, colour=CITY)) + 
      ...
      scale_y_continuous(breaks = pretty(temp_plot$TMAX_mean, n = 5)) +
      labs(
        x = 'Week',
        y = 'Celcius',
...
CITY <- c("HANOI/SF/OAKLAND", "STPETER", "SWARTHMORE", "VICTORIA")
...
snow_plot <- data.frame(CITY, SNOW_TOTALS, SNOW_DAYS)

ggplot(snow_plot, aes(x=SNOW_TOTALS, y=SNOW_DAYS, label=CITY)) + 
  geom_point(size=3) +
...
ggplot(data_plot, aes(x=WEEK, y=PRCP_mean, group=CITY, colour=CITY)) +
      geom_ribbon(aes(ymin=0, ymax=PRCP_mean, fill=CITY), alpha=0.6, linetype=0) +
      scale_x_continuous(breaks = c(1, 14, 27, 40, 52),
                         labels = c("1"="1 (Jan)", "14"="14 (Apr)", "27"="27 (Jul)", "40"="40 (Oct)", "52"="52 (Dec)")) + 
      scale_y_continuous(breaks = pretty(data_plot$PRCP_mean, n = 5)) +
      labs(
        x = 'Week',
        y = 'mm',
